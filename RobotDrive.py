...
class RobotDrive():
    ...
    global test2
    test2 = TempCollector.SensorCollector()
    ...
    def motor_drive (self, value):                                                # Accept the motor drive value
    ...
    def back_away (self, direction):
        movement = self.motor_drive (False)                                      # First of all, stop the car
        time.sleep (0.5)                                                    # Wait for half a second
        distance = self.get_distance()                                           # Get the current object distance
        while distance < 20:                                                # Whilst it's less than 20cm
            movement = self.motor_drive (direction)                              # keep reversing the car
            distance = self.get_distance ()                                      # and checking the distance
        movement = self.motor_drive (False)                                      # Otherwise, stop the car,
        time.sleep (0.5)                                                    # wait for half a second,
    ...
    def automatic (self, buttons):
        ...
        while (autoDrive == True):
            ...
            distance = self.get_distance()                                      # Check the distance
            if direction == fwd and distance < 15:                          # If the distance is less than 15cm then
                movement = self.back_away(rev)                                  # reverse the car if driving forward
                ...
                movement = self.motor_drive(direction)                          # Now turn in that direction
                time.sleep (0.5)                                            # for half a second
                movement = self.motor_drive(stop)                               # Then stop for half a second
                time.sleep (0.5)
            movement = self.motor_drive(direction)                              # Continue moving in a forwards direction
    ...
    def exit_program(self):
        exitFlag = True
        z = self.motor_drive (False)                                             # Turn off all the GPIO outputs
        ...
        while True:                                                             # Start an infinite loop
            try:
                ...
                if word == 3:                                                   # If it fails after 3 attempts...
                    ...
                    terminate = self.exit_program()                                  # And exit the program
    ...
    def moveRobot(self):
        ...
        while True:                                                             # Begin an infinite loop
            ...
            if (buttons - cwiid.BTN_PLUS - cwiid.BTN_MINUS == 0):               # Are both the '+' and '-' buttons pressed?
                ...
                wii.rumble = 0
                sensorThread.daemon()
                terminate = self.exit_program()                                      # And quit the program


            if (buttons - cwiid.BTN_A == 0):                                    # If ONLY the 'A' button is pressed
                movement = self.automatic(buttons)
        ...
        # *******************************************************************
            ...
            distance = self.get_distance ()                                          # Get object distance in centimetres

            if direction == fwd and distance < 15:                              # If the distance is less than 15cm then
                direction = self.back_away (rev)                                     # reverse the car if driving forward

            movement = self.motor_drive (direction)                                  # Otherwise get the car moving (if needs be)

    def mergeDriveAndSensor(self):
        test2.getConstantStreamOfData()
    ...
    def setUpThreads(self):
        global sensorThread
        moveThread  = Thread(target = test1.moveRobot)
        sensorThread = Thread(target = test1.mergeDriveAndSensor)
        moveThread.start()
        sensorThread.start()
